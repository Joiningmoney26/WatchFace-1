...
static void main_window_load(Window *window) {
  ...
  s_background_bitmap = gbitmap_create_with_resource(RESOURCE_ID_SPACE);
  s_background_layer = bitmap_layer_create(GRect(0, -6, 144, 180));
  bitmap_layer_set_bitmap(s_background_layer, s_background_bitmap);
  layer_add_child(window_get_root_layer(window), bitmap_layer_get_layer(s_background_layer));
  
  // Create time TextLayer
  s_time_font = fonts_load_custom_font(resource_get_handle(RESOURCE_ID_AIRSTRIKE_40));

  //time layer
  s_time_layer = text_layer_create(GRect(5, 100, 120, 40));
